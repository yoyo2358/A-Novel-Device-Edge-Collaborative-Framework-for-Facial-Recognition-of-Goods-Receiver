...
if __name__=="__main__":
    ...
    parser.add_argument('-batch_size', type=int, default=1, help='batch size for training [default: 8]')
    ...
    # model
    parser.add_argument('-mode', default='gensingle', help=' trainrandom | trainsingle | trainmulti | gensingle | genmulti | idensingle | idenmulti')
    parser.add_argument('-init_type', type=str, default='kaiming', help='the type of initializing model: normal | xavier | kaiming | orthogonal')
    ...
    parser.add_argument('-modeldir', type=str, default='./snapshot/bestmodel', help='filename of model to test(snapshot/{Single or Multiple}/{date}) [default: None]')
    ...
    # data source
    parser.add_argument('-data_path', type=str, default='./100face.txt', help='path to filelist')
    ...
    print("args = parser.parse_args() has finished")
    dataloader = initdataloader(args)
    print("dataloader = initdataloader(args) has finished")
    netD, netG = initmodel(args)
    print("netD, netG = initmodel(args) has finished")
    message = initrun(dataloader, netD, netG, args)
    print("message = initrun(dataloader, netD, netG, args) has finished")
    print(message)
    end = time.time()
    print("Face Rotate successfully finished. Total time: " + str(end - start) + " seconds")
