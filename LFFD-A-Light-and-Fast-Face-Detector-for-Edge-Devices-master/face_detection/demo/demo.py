...
def parse_args():
    ...
    parser.add_argument('--use-gpu', type=bool, default=False,
                        help='Default is cpu.')
    parser.add_argument('--data', type=str, default='F:/Mydataset/video_flames',
                        help='The path of input file.')  #需要更改这里，也就是图片的路径
...
def main():
    ...
    if args.version == 'v1':
        ...
        model_file_path = '../saved_model/configuration_10_320_20L_5scales_v2/train_10_320_20L_5scales_v2_iter_1000000.params'
    else:
    ...
    if args.mode == 'image':
        ...
        for file_name in file_name_list:
            ...
            for bbox in bboxes:
                ...
                cv2.imwrite('F:/Mydataset/detected_crop_faces/' + str(index) + '.jpg', cropped)
                # cropped.save('F:/test/face_detect/crop/50/' + str(index) + '.jpg')
                print("保存剪切图片成功！")
                index = index + 1
            ...
            #cv2.imshow('im', im)
            #cv2.waitKey(5000)#注意保存图片的时候文件夹要更改一下
            out_folder='F:\Mydataset\detected_uncrop_faces'
            cv2.imwrite(os.path.join(out_folder, file_name.replace('.jpg', '_result.png')), im)
            print('当前第'+str(account)+'张图片')
            account = account + 1
